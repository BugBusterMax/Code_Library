...
for i in range(8640):
    features[i] += gen_gaussian_noise(features[i], 30)
for i in range(8640):
    for j in range(6):
        ssp = random.random()
        ssr = float(1/6)
...
labels2_test = labels2[6912:8640, :]
def MAE(SR, GT):
    SR = SR.flatten()
    GT = GT.flatten()
...
def train(args, model, optimizer):
    ...
    for epoch in range(args.EPOCH):
        model.train()
        for step, (x, y) in enumerate(zip(idx_train, labels2_train)):
...
def validate(model, epoch):
    ...
    with torch.no_grad():
        ...
        MSE_all = []
        R_all = []
        for x, y in zip(idx_test, labels2_test):
            x, y = torch.FloatTensor(x), torch.FloatTensor(y)
            x, y = x.cuda(), y.cuda()
            predict = model(x)
            label = y.squeeze()
        ...
        with open('LR_label2_30DB.csv', 'a') as f:
            f.write(str_ + '\n')
